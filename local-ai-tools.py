...
def health():
    ...
    for m in running:
        ...
        detail = (
            f"{human_bytes(size_vram)} VRAM"
            + (f" ({pct:.0f}% GPU-resident)" if pct is not None else "")
            + f", ctx {m.get('context_length', 'n/a')}"
        )
    ...
    if gpu:
        print(
            f"GPU: {gpu['util']}% util | {gpu['vram_used']} / {gpu['vram_total']} MiB VRAM | "
            f"{gpu['temp']} C | {gpu['power']} W"
        )
    if mem:
        print(
            f"RAM: {human_bytes(mem['used'])} used / {human_bytes(mem['total'])} | "
            f"{human_bytes(mem['available'])} available | swap {human_bytes(mem['swap_used'])}"
        )
...
def models():
    ...
    for m in installed:
        ...
        print(
            f"{name[:44]:44} "
            f"{human_bytes(m.get('size')):>10} "
            f"{str(d.get('parameter_size','')):>9} "
            f"{str(d.get('quantization_level','')):>10} "
            f"{state:>10}"
        )
...
def tps(out, ns):
    return out / (ns / 1e9) if ns else None
    ...
    while True:
        ...
        try:
            ...
            for label, cutoff in [
                ...
                print(
                    f"{label:<10} in {a['in']:>11,} | out {a['out']:>11,} | "
                    f"total {a['in']+a['out']:>11,} | responses {a['count']:>6,} | {fmt_rate(rate)}"
                )
            ...
            for model, x in sorted(by_model.items(), key=lambda kv: kv[1]["in"] + kv[1]["out"], reverse=True):
                ...
                print(
                    f"{model}\n"
                    f"  input {x['in']:,} | output {x['out']:,} | total {x['in']+x['out']:,} | "
                    f"reasoning {x['reasoning']:,} | responses {x['count']:,} | {fmt_rate(rate)}"
                )
...
def benchmark(model, num_ctx=8192, runs=3, output_tokens=128):
    ...
    prompt = (
        "Explain why increasing an LLM's context window increases KV-cache memory usage. "
        "Be technically precise, use exactly four short paragraphs, and do not use bullet points."
    )
    ...
    for idx in range(1, runs + 1):
        ...
        prompt_tps = pin / (pd / 1e9) if pd else None
        output_tps = pout / (ed / 1e9) if ed else None
        ...
        row = {
            ...
            "size_vram": size_vram,
            "context_length_live": ctx_live,
            "gpu_before": gpu_before,
            "ram_before_available": ram_before.get("available") if ram_before else None,
        }
        results.append(row)

        print(
            f"run {idx}: prompt {pin:,} @ {fmt_rate(prompt_tps)} | "
            f"output {pout:,} @ {fmt_rate(output_tps)} | "
            f"wall {wall:.2f}s | VRAM {human_bytes(size_vram)}"
        )
...
def benchmark_history(limit=20):
    ...
    for r in rows[-limit:]:
        ...
        print(
            ...
            f"{fmt_rate(pt) if pt is not None else 'n/a':>12} "
            f"{fmt_rate(ot) if ot is not None else 'n/a':>12} "
            f"{human_bytes(r.get('size_vram')):>10}"
        )
...
def dashboard():
    while True:
        ...
        print(
            f"Ollama: {service_state('ollama.service')} | "
            f"Open WebUI: {service_state('open-webui.service')} | "
            f"WebUI health: {'OK' if webui_health() else 'FAIL'}"
        )
        ...
        if gpu:
            print()
            print(
                f"GPU: {gpu['name']} | {gpu['util']}% util | "
                f"{gpu['vram_used']}/{gpu['vram_total']} MiB | "
                f"{gpu['temp']} C | {gpu['power']} W"
            )

...
>>"%BOOTLOG%" echo.
>>"%BOOTLOG%" echo ============================================================
>>"%BOOTLOG%" echo [%DATE% %TIME%] Bootstrap action: benchmark
>>"%BOOTLOG%" echo Script dir: %SCRIPT_DIR%

echo Local AI bootstrap
echo Action: benchmark
echo.
...
start "Local AI PowerShell" "%PWSH%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%RUNNER%" -Controller "%CONTROLLER%" -Action benchmark
